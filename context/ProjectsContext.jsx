...
export const ProjectsProvider = ({children}) => {
  ...
  const addTaskToProject = (projectName, task) => {
    try {
      const projectRef = database().ref('/projects').child(projectName);

      // Ensure that the project exists before adding the task
      projectRef.once('value', snapshot => {
        if (snapshot.exists()) {
          // Get the current tasks array and add the new task to it
          const currentTasks = snapshot.val().tasks || [];

          // Add the new task to the tasks array
          currentTasks.push(task);

          // Update the tasks list in the project
          projectRef
            .update({tasks: currentTasks})
            .then(() => {
              console.log('Task added successfully to project:', projectName);
            })
            .catch(error => {
